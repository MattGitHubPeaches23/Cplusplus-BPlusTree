...
template<typename T>
BPlusTree<T>::BPlusTree(const BPlusTree<T>& rhs):data_count(0), child_count(0){

    BPlusTree<T> *ptr;
    ptr = rhs.subset[0];
    bool over = false;
    do{
        if(ptr->subset[0] == NULL){
            over = true;
        }
        else if(ptr->subset[0] != NULL){
            ptr = ptr->subset[0];
        }
...
//    //Copy children
//    if(0 != child_count){
//        for(int i = rhs.child_count - 1; i >= 0; --i){
//            subset[i] = new BPlusTree<T>;
//            copy_tree_2(subset[i],rhs.subset[i]);
//            if(i != rhs.child_count - 1){
//                BPlusTree<T> *front, *back;
//                if(0 == subset[i]->child_count){
//                    front = subset[i];
//                }
